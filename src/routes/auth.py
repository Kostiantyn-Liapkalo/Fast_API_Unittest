...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    ...
    :return: The user object, which contains the email and username of the new account
    """
...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    :param body: OAuth2PasswordRequestForm: Get the user's email and password
...
async def confirmed_email(token: str, db: Session = Depends(get_db)):
    ...
    :param db: Session: Get the database session
    :return: A message if the user is already confirmed
    """
...
async def request_email(body: RequestEmail, background_tasks: BackgroundTasks, request: Request,
    ...
    :param background_tasks: BackgroundTasks: Add a task to the background tasks queue
    :param request: Request: Get the base url of the application, which is used to generate a link
    :param db: Session: Get the database session
    :return: A message that is shown to the user
    """
